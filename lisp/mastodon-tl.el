...
(defun mastodon-tl--timeline (toots)
  ...
  (let ((insert-point (or mastodon-tl--update-point (point-min))))
    (mapc (lambda (toot)
...
(defun mastodon-tl--thread-after-load (success _headers context id toot)
...
(defun mastodon-tl--more-after-load (success _headers body buffer update-function)
...
(defun mastodon-tl--update ()
  ...
  (let* ((endpoint (mastodon-tl--get-endpoint))
    ...
    (mastodon-http--get-json-async
     url
     (lambda (success _headers json buffer)
...
(defun mastodon-tl--init-after-load
    (success _headers body buffer buffer-name endpoint update-function)
